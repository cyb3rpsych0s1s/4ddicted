...
private func RemoteLogChannel(message: String) -> Void { LogChannel(n"DEBUG", s"[audioware:rust] \(message)"); }
public static func DEBUG(message: String) -> Void {
    LogChannel(n"DEBUG", s"[audioware:reds] \(message)");
}
public static func ASSERT(message: String) -> Void {
    LogChannel(n"ASSERT", s"[audioware:reds] \(message)");
}
